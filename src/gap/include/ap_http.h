...
protected:
  ...
  virtual FXival read(void*,FXival);
private:
...
protected:
  virtual FXival io_read(void*,FXival);
  virtual FXival io_write(const void*,FXival);
  virtual FXbool io_wait_write() { return false; }
  virtual FXbool write(const void*,FXival);
protected:
  FXbool open_connection();
  void reset(FXbool forceclose);
protected:
  HttpClient(MemoryBuffer* b);
public:
  HttpClient();
