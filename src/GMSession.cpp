...
********************************************************************************
*           Copyright (C) 2012-2012 by Sander Jansen. All Rights Reserved      *
*                               ---                                            *
...
public:
  ...
  FXbool init(int argc,const FXchar * const argv[]) {
    ...
    if (connection) {
      ...
      fxmessage("connected to %s client id %s\n",SmcVendor(connection),clientid);

//      fxmessage("setting properties");
//
      setProperty("_GSM_DesktopFile","/usr/share/applications/gogglesmm.desktop");
      setProperty("Priority","30");

      //setProperty("_GSM_DesktopFile","gogglesmm");
      setProperty(SmCurrentDirectory,FXSystem::getCurrentDirectory().text());
      setProperty(SmProcessID,FXString::value(FXProcess::current()).text());
      setProperty(SmUserID,FXSystem::currentUserName().text());
      setProperty(SmProgram,"gogglesmm");

      FXStringList cmd;
      cmd.append("gogglesmm");

      setProperty(SmCloneCommand,cmd);
      setProperty(SmRestartCommand,cmd);
 //
...
static void ice_init() {
  ...
  // Register with Session Manager if any
#if defined(HAVE_SM) && defined(HAVE_ICE)
  if (!session->init(getArgc(),getArgv())) {
    delete session;
    session=NULL;
    delete icehook;
    icehook=NULL;
