...
//  target_show_albumcovers.connect(GMPlayerManager::instance()->getPreferences().gui_show_albumcovers);
  ...
  target_dbus_mpris.connect(GMPlayerManager::instance()->getPreferences().dbus_mpris);
#endif
...
//  new GMCheckButton(grpbox,tr("Show album covers in album browser\tShow album covers in album browser"),&target_show_albumcovers,FXDataTarget::ID_VALUE);
  ...
  if (GMPlayerManager::instance()->hasSessionBus()) {
    ...
    new GMCheckButton(grpbox,tr("MPRIS Connectivity\tEnable MPRIS connectivity"),&target_dbus_mpris,FXDataTarget::ID_VALUE);
    }
#endif
...
*/
  ...
  new FXColorWell(matrix,0,this,ID_HILITE_COLOR,COLORWELL_OPAQUEONLY|FRAME_SUNKEN|FRAME_THICK|LAYOUT_FIX_WIDTH|LAYOUT_FIX_HEIGHT|LAYOUT_FILL_ROW|LAYOUT_CENTER_Y,0,0,40,24);
...
long GMPreferencesDialog::onCmdAccept(FXObject*,FXSelector,void*) {
  ...
  if (GMPlayerManager::instance()->getPreferences().gui_show_playing_albumcover) {
    ...
    mainwindow->loadCover(FXString::null);
    }
