...
FXuint ID3V2::parse_frame(const FXuchar * buffer, const FXint size, const FXint version) {
  ...
  if (!skip && framesize) {
    ...
    switch(frameid) {
      ...
      // case PRIV : parse_priv_frame(static_cast<FXint>(framesize));
      //             break;
      case COMM :
        {
          FXString key, value;
...
ID3V2 * ID3V2::parse(InputPlugin * input,const FXuchar * id, FXbool skip) {
  ...
  //
  // // Major Version
  // version = static_cast<FXchar>(buffer[3]);
  //
  // // Flags
  // const FXuchar & flags = buffer[4];
  //
  ...
  // const FXuchar & flags = info[1];
  //
  // FXuint tagsize = ID3_SYNCSAFE_UINT32(info+2);
  //
  // // Prevent abuse
  // if (tagsize > ID3V2_MAX_SIZE) {
  //   return nullptr;
  // }
  //
  // FXuint minsize = 10;
  ...
  // if (!allocElms(tagbuffer,tagsize)) {
  //   return nullptr;
  //   }
  //
...
// ID3V2::ID3V2(FXuchar *b,FXint len) : buffer(b),size(len){
//   if (size < 10)
//     return;
//
//   // Major Version
...
// void ID3V2::unsync(FXuchar * src,FXuint & len) {
//   FXuint k=0;
//   for (FXuint i=0;i<len;i++) {
//     src[k++]=src[i];
//     if (src[i]==0xff && (i+1)<len && src[i+1]==0x00)
...
//
//
// void ID3V2::parse_comment_frame(FXuint framesize) {
//   FXString key,field;
//
...
//     return;
//
//   const FXuchar & encoding = buffer[p + 0];
//   const FXchar * textstart = reinterpret_cast<const FXchar*>(buffer + p + 1);
//
//   parse_text(encoding, textstart,framesize - 1,text);
//   GM_DEBUG_PRINT("[id3v2] text: \"%s\"\n",text.text());
//
//   switch(frameid) {
//     case TP1  :
...
//     framesize = (buffer[p+3]<<16) | (buffer[p+4]<<8) | (buffer[p+5]);
//     p+=6;
//
//     if (size - p < framesize)
//       goto fail;
...
//     }
//
//     if (size - p - 10 < extra || size - p - 10 - extra < framesize)
//       goto fail;
//
//     p += 10 + extra;
//   }
//
//   if (!skip && framesize > 0 && framesize < ID3V2_MAX_SIZE) {
//     switch(frameid) {
//       case TP1  :
//       case TPE1 :
//       case TAL  :
//       case TALB :
//       case TT2  :
//       case TIT2 : parse_text_frame(frameid, static_cast<FXint>(framesize)); break;
//       case RVA2 : parse_rva2_frame(static_cast<FXint>(framesize)); break;
//       case PRIV : parse_priv_frame(static_cast<FXint>(framesize)); break;
//       case COMM : parse_comment_frame(static_cast<FXint>(framesize)); break;
